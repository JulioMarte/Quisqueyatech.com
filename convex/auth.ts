...
export const listAgents = query({ args: { secret: v.string() }, handler: async (ctx, args) => { await requireServer(ctx, args.secret); return (await ctx.db.query("contentAgents").order("desc").take(500)).map(agent => ({ _id: agent._id, keyId: agent.keyId, name: agent.name, prefix: agent.prefix, status: agent.status, requestLimit: agent.requestLimit, uploadLimit: agent.uploadLimit, createdBy: agent.createdBy, createdAt: agent.createdAt, rotatedAt: agent.rotatedAt, revokedAt: agent.revokedAt, lastUsedAt: agent.lastUsedAt })); } });
