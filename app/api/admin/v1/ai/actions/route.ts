...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (idempotencyKey)
      previous = await fetchAuthQuery(api.posts.adminIdempotencyGet, {
        scope: "content-ai",
        key: idempotencyKey,
      });
  } catch (error) {
    return adminException(trace, "content-ai.authorize", error);
  }
  if (previous) return adminJson(trace, previous);
  try {
    const rate = await checkSecurityRateLimit(
      requestFingerprint(request, "content-ai"),
      30,
      60 * 60 * 1000,
    );
    if (!rate.allowed) return adminFailure(trace, "AI request limit reached", 429);
  } catch (error) {
    return adminException(trace, "content-ai.rate-limit", error);
  }
  let payload: unknown;
  try {
    payload = await readAdminJson(request, 140_000);
  } catch (error) {
    return adminException(trace, "content-ai.payload", error);
  }
  ...
  try {
    ...
    const runId = await fetchAuthMutation(api.posts.adminRecordAiRun, {
      postId: parsed.data.postId as Id<"posts"> | undefined,
      action: parsed.data.action,
      provider: "gemini",
      model: generated.model,
      status: "completed",
      warnings: generated.proposal.warnings,
    ...
    if (idempotencyKey)
      await fetchAuthMutation(api.posts.adminIdempotencyPut, {
        scope: "content-ai",
        key: idempotencyKey,
        value: data,
      });
    ...
    try {
      await fetchAuthMutation(api.posts.adminRecordAiRun, {
        postId: parsed.data.postId as Id<"posts"> | undefined,
        action: parsed.data.action,
        provider: "gemini",
        model: process.env.CONTENT_AI_MODEL || "gemini-2.5-flash",
        status: "failed",
        warnings: ["AI provider failed"],
        durationMs: Date.now() - started,
      });
    } catch {
      /* storage may be unavailable too */
    }
    if (/not configured|unsupported content AI provider/i.test(message))
      return adminException(trace, "content-ai.generate", error);
    console.error(
      JSON.stringify({
        scope: "admin-api",
        requestId: trace,
        operation: "content-ai.generate",
