...
const progressTool = {
  temporaryTool: {
    ...
    description:
      "Persist newly learned or corrected assessment facts. Call after every substantive user answer and follow the returned private next-step instruction.",
    dynamicParameters: [
      {
        name: "reason",
        location: "PARAMETER_LOCATION_BODY",
        schema: { type: "string", enum: ["answer", "correction", "interruption", "close"] },
        required: true,
      },
      {
        name: "updates",
        ...
        schema: {
          ...
          items: {
            ...
            properties: {
              ...
              status: { type: "string", enum: ["confirmed", "estimated", "inferred", "pending"] },
...
abstract class BaseAdapter implements VoiceProviderAdapter {
  ...
  async sendGuidance() {
    throw new Error(`${this.id} does not support server-side guidance`);
  }
  async endSession() {
    throw new Error(`${this.id} does not support server-side termination`);
  }
  async getCanonicalTranscript(): Promise<TranscriptTurn[]> {
    throw new Error(`${this.id} does not expose a canonical transcript through this adapter`);
  }
  normalizeLifecycleEvent(payload: unknown): NormalizedVoiceEvent {
    ...
    const type =
      event.includes("error") || event.includes("fail")
        ? "error"
        : event.includes("end") || event.includes("disconnect")
          ? "ended"
          : event.includes("join") || event.includes("connect")
            ? "joined"
            : "started";
    return {
      type,
      providerSessionId: String(value.callId || value.id || "") || undefined,
...
class UltravoxAdapter extends BaseAdapter {
  ...
  async createSession(context: AssessmentContext): Promise<ProviderSession> {
    ...
    const response = await fetch(
      String(config.ultravoxApiUrl || "https://api.ultravox.ai/api/calls"),
      {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          timeExceededMessage:
            context.locale === "es"
              ? "Hemos llegado al límite de tiempo. Gracias; recibirás tu reporte revisado por correo."
              : "We have reached the time limit. Thank you; your reviewed report will arrive by email.",
          inactivityMessages: [
            {
              duration: "25s",
              message:
                context.locale === "es"
                  ? "¿Sigues ahí? Tómate tu tiempo."
            ...
            },
            {
              ...
              message:
                context.locale === "es"
                  ? "Podemos retomar durante las próximas 24 horas. Cerraré por ahora."
                  : "You can resume within 24 hours. I will close for now.",
...
class GeminiLiveAdapter extends BaseAdapter {
  ...
  async createSession(context: AssessmentContext): Promise<ProviderSession> {
    ...
    return {
      ...
      model,
      sessionConfig: { responseModalities: ["AUDIO"], language: context.locale, systemInstruction },
    };
  }
}

const adapters: Record<VoiceProviderId, VoiceProviderAdapter> = {
  ultravox: new UltravoxAdapter(),
...
export function getVoiceProvider(provider: VoiceProviderId): VoiceProviderAdapter {
