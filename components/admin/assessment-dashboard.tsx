...
import { ReportEditor, type AssessmentReport } from "@/components/admin/assessment-admin";
import { parseAssessmentTranscript } from "@/lib/assessment/transcript";

type EvidenceStatus = "confirmed" | "estimated" | "inferred" | "pending";
type FieldKey =
  | "name"
  | "company"
  | "role"
...
type Detail = Summary & {
  ...
  snapshot?: {
    revision: number;
    fields: Partial<Record<FieldKey, EvidenceField>>;
...
export function AssessmentAdmin() {
    ...
    async (append = false, nextCursor: string | null = null) => {
      ...
      try {
        const page = await adminRequest<AdminPage<Summary>>(
          `/api/admin/v1/assessments?limit=25${reportFilter ? `&status=${encodeURIComponent(reportFilter)}` : ""}${nextCursor ? `&cursor=${encodeURIComponent(nextCursor)}` : ""}`,
          { cache: "no-store" },
        );
        ...
        announce(error, "No se pudieron cargar las evaluaciones.");
      } finally {
        setLoading(false);
      }
    },
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      if (assessmentFilter && item.status !== assessmentFilter) return false;
      if (!needle) return true;
      return [item.lead?.firstName, item.lead?.lastName, item.lead?.company, item.lead?.email]
        .filter(Boolean)
        .join(" ")
        .toLocaleLowerCase("es")
        .includes(needle);
    });
  }, [assessmentFilter, items, query]);

  function announce(error: unknown, fallback: string, kind: "error" | "success" = "error") {
    setMessage({ text: error instanceof Error ? error.message : fallback, kind });
  }
  async function open(item: Summary, nextTab: Tab = "summary") {
    setLoading(true);
    setMessage(null);
    try {
      ...
      setDraftFields(detail.snapshot?.fields || {});
      setPreviewHtml("");
      setEditing(nextTab === "data");
      setTab(nextTab);
      requestAnimationFrame(() => document.getElementById("assessment-detail")?.focus());
    } catch (error) {
      announce(error, "No se pudo cargar la evaluación.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function saveFields() {
    ...
    try {
      ...
      await adminRequest(`/api/admin/v1/assessments/${selected.assessmentId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ expectedRevision: selected.snapshot.revision, fields }),
      });
      setEditing(false);
      setMessage({ text: "Datos de la evaluación actualizados.", kind: "success" });
      await refreshSelected();
    } catch (error) {
      announce(error, "No se pudieron guardar los datos.");
    } finally {
      setBusy(false);
    }
  }
  async function review(action: "save" | "approve-and-send") {
    ...
    try {
      ...
      announce(error, "No se pudo completar la revisión.");
    } finally {
      setBusy(false);
    }
  }
  async function preview() {
    ...
    try {
      ...
      announce(error, "No se pudo generar la vista previa.");
    }
  }
  async function remove() {
    if (!selected || deleteText !== selected.assessmentId) return;
    setBusy(true);
    try {
      await adminRequest(`/api/admin/v1/assessments/${selected.assessmentId}`, {
        method: "DELETE",
      });
      const removedId = selected.assessmentId;
      setItems((current) => current.filter((item) => item.assessmentId !== removedId));
      setSelected(null);
      setDeleteText("");
      deleteRef.current?.close();
      setMessage({
        text: "Evaluación y datos asociados eliminados definitivamente.",
        kind: "success",
      });
    } catch (error) {
      announce(error, "No se pudo eliminar la evaluación.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Section className="min-h-screen bg-bg-2 py-8">
      <Container className="max-w-[1600px]">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-text-2">
              Gestiona la evidencia, corrige los datos recopilados y aprueba reportes desde un solo
              espacio.
            </p>
          </div>
        ...
        <section
          className="mt-7 rounded-2xl border border-line bg-white shadow-sm"
        ...
        >
          ...
          <AssessmentTable
            ...
            onOpen={open}
            onDelete={(item) => {
              void open(item).then(() => deleteRef.current?.showModal());
            }}
          />
          {!isDone ? (
            <div className="border-t border-line p-4 text-center">
              <Button variant="outline" disabled={loading} onClick={() => void load(true, cursor)}>
                Cargar más
              </Button>
            </div>
          ) : null}
        </section>
        <main id="assessment-detail" tabIndex={-1} className="mt-6 focus:outline-none">
          {selected ? (
            <DetailPanel
              detail={selected}
              tab={tab}
            ...
            <div className="flex min-h-56 items-center justify-center rounded-2xl border border-dashed border-line bg-white text-text-2">
...
function AssessmentTable({
  ...
  items: Summary[];
  loading: boolean;
  onOpen(item: Summary, tab?: Tab): void;
  onDelete(item: Summary): void;
}) {
  if (loading && !items.length)
    return <p className="p-10 text-center text-text-2">Cargando evaluaciones…</p>;
  if (!items.length)
    return (
    ...
    <>
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.assessmentId} className="border-t border-line hover:bg-bg-2/70">
                <td className="px-4 py-4">
                  <p className="font-semibold text-primary">{fullName(item)}</p>
                  <p className="mt-1 text-xs text-mute">
                    {item.lead?.company || "Sin empresa"} · {item.lead?.email || "Sin correo"}
                  </p>
                </td>
                <td className="px-4 py-4">
                  ...
                  <p className="mt-1 text-xs text-mute">
                    {item.completedAt
                      ? formatDurationMs(item.completedAt - item.createdAt)
                      : "En curso"}
                  </p>
                </td>
                <td className="px-4 py-4">
                  <StatusBadge value={item.status} kind="assessment" />
                </td>
                <td className="px-4 py-4">
                  <StatusBadge value={item.reportStatus || "collecting"} kind="report" />
                </td>
                <td className="px-4 py-4">
                  <Coverage value={item.coverageScore || 0} />
                </td>
                <td className="px-4 py-4">
      ...
      <div className="grid gap-3 p-3 md:hidden">
        {items.map((item) => (
          <article key={item.assessmentId} className="rounded-xl border border-line p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="font-semibold text-primary">{fullName(item)}</h3>
                <p className="mt-1 text-sm text-text-2">{item.lead?.company || "Sin empresa"}</p>
              </div>
              <Coverage value={item.coverageScore || 0} />
            </div>
            ...
            <p className="mt-3 text-xs text-mute">
              {formatDate(item.createdAt)} · {item.lead?.email}
            </p>
            <div className="mt-4 grid grid-cols-3 gap-2">
              <Button variant="outline" onClick={() => onOpen(item)}>
                Abrir
              </Button>
              <Button variant="outline" onClick={() => onOpen(item, "data")}>
                Editar
              </Button>
              <Button variant="outline" className="text-rose-800" onClick={() => onDelete(item)}>
                Eliminar
              </Button>
            </div>
          </article>
  ...
  return (
    <label className="text-xs font-semibold text-mute">
      ...
      <select
        ...
        className="min-h-11 w-full rounded-lg border border-line bg-white px-3 text-sm font-medium text-text"
      >
...
function IconButton({
  ...
  return (
    <button
      ...
      title={label}
      onClick={onClick}
      className={`flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-bg-3 ${danger ? "text-rose-800" : "text-primary"}`}
    >
      {icon}
    </button>
  );
