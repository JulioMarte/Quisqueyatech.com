...
export const setResumeCredential = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(item._id, {
      resumeTokenHash: args.tokenHash,
      resumeExpiresAt: args.expiresAt,
    });
  },
});
...
export const checkRateLimit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const item = await ctx.db
      .query("assessmentRateLimits")
      .withIndex("by_key", (q) => q.eq("key", args.key))
      .unique();
    if (!item || item.resetAt <= args.now) {
      if (item) await ctx.db.patch(item._id, { count: 1, resetAt: args.now + args.windowMs });
      else
        await ctx.db.insert("assessmentRateLimits", {
          key: args.key,
          count: 1,
    ...
    if (item.count >= args.limit) return false;
    await ctx.db.patch(item._id, { count: item.count + 1 });
    return true;
  },
});
...
export const consumeResumeCredential = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const item = await assessmentById(ctx, args.assessmentId);
    if (
      !item.resumeTokenHash ||
      item.resumeTokenHash !== args.tokenHash ||
      !item.resumeExpiresAt ||
      item.resumeExpiresAt <= args.now
    )
      return null;
...
export const advance = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const prior = await ctx.db
      ...
      .withIndex("by_assessment_event", (q) =>
        q.eq("assessmentId", args.assessmentId).eq("eventId", args.input.eventId),
      )
      .unique();
    ...
    await ctx.db.patch(item._id, {
      ...
      status: output.snapshot.complete
        ? "interview_complete"
        : args.input.reason === "interruption"
          ? "interrupted"
          : "in_progress",
    ...
    if (fields.phone?.status === "confirmed")
      leadPatch.phone = fields.phone.value.replace(/[\s()-]/g, "");
...
export const getState = query({
  args: { ...serviceArgs, assessmentId: v.string() },
  handler: async (ctx, args) => {
    requireService(args.serviceSecret);
...
export const getByProviderSession = query({
  args: { ...serviceArgs, providerSessionId: v.string() },
  handler: async (ctx, args) => {
    requireService(args.serviceSecret);
    const session = await ctx.db
      .query("assessmentSessions")
      .withIndex("by_provider_session", (q) => q.eq("providerSessionId", args.providerSessionId))
      .unique();
    if (!session) return null;
    const item = await assessmentById(ctx, session.assessmentId);
    return { ...item, session, lead: await ctx.db.get(item.leadId) };
  },
});
export const storeSessionReport = mutation({
  args: {
    ...serviceArgs,
    sessionKey: v.string(),
    transcript: v.string(),
...
export const recordWebhook = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      (existing?.status === "processing" && existing.receivedAt > args.receivedAt - 5 * 60_000)
    )
      return false;
    if (existing) {
...
export const claimFinalization = mutation({
  args: { ...serviceArgs, assessmentId: v.string(), now: v.number() },
  handler: async (ctx, args) => {
    requireService(args.serviceSecret);
    const item = await assessmentById(ctx, args.assessmentId);
...
export const failFinalization = mutation({
  args: { ...serviceArgs, assessmentId: v.string(), error: v.string() },
  handler: async (ctx, args) => {
    requireService(args.serviceSecret);
    const item = await assessmentById(ctx, args.assessmentId);
...
export const complete = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("voiceMetrics", {
      ...
      provider: args.provider,
      completion: true,
      extractionScore: assessment.coverageScore,
...
export const adminList = query({
  args: { paginationOpts: paginationOptsValidator, reportStatus: v.optional(v.string()) },
  handler: async (ctx, args) => {
    await requireAdminIdentity(ctx);
    const result = args.reportStatus
      ? await ctx.db
          .query("assessments")
          .withIndex("by_report_status", (q) => q.eq("reportStatus", args.reportStatus))
          .order("desc")
          .paginate(args.paginationOpts)
      : await ctx.db.query("assessments").order("desc").paginate(args.paginationOpts);
    return {
      ...result,
      page: await Promise.all(
        result.page.map(async (item) => {
          ...
          return {
            ...
            reportRevision: item.reportRevision ?? 0,
            stage: item.stage,
            coverageScore:
              item.coverageScore ??
...
export const adminGetState = query({
...
export const adminCorrectSnapshot = mutation({
  args: {
    ...
    fields: v.array(adminFieldValidator),
  },
  handler: async (ctx, args) => {
    ...
    if (fields.role?.status === "confirmed") leadPatch.role = fields.role.value;
    if (fields.email?.status === "confirmed") leadPatch.email = fields.email.value;
    if (fields.phone?.status === "confirmed") leadPatch.phone = fields.phone.value;
    await ctx.db.patch(item.leadId, leadPatch);
    return { revision: output.snapshot.revision, coverageScore: output.coverageScore };
  },
});
...
export const adminReview = mutation({
  ...
  handler: async (ctx, args) => {
    const admin = await requireAdminIdentity(ctx);
    const item = await assessmentById(ctx, args.assessmentId);
    if ((item.reportRevision ?? 0) !== args.expectedRevision)
      throw new Error("CONFLICT: stale assessment revision");
    if (item.reportStatus === "sending" && (item.reportSendClaimExpiresAt ?? 0) > Date.now())
      throw new Error("CONFLICT: report is sending");
    if (item.reportStatus === "sent") throw new Error("CONFLICT: sent reports are immutable");
    const now = Date.now();
...
export const claimReportSend = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const item = await assessmentById(ctx, args.assessmentId);
    const now = Date.now();
    if (item.reportStatus === "sent") throw new Error("CONFLICT: report already sent");
    if (item.reportStatus === "sending" && (item.reportSendClaimExpiresAt ?? 0) > now) {
      if (
        item.reportContentHash === args.contentHash &&
        item.reportSendKey &&
        item.reportSendClaimId
      )
        return {
          claimId: item.reportSendClaimId,
          idempotencyKey: item.reportSendKey,
          revision: item.reportRevision ?? 0,
        };
      throw new Error("CONFLICT: report is sending");
    }
    if ((item.reportRevision ?? 0) !== args.expectedRevision)
    ...
    const idempotencyKey =
      item.reportContentHash === args.contentHash && item.reportSendKey
        ? item.reportSendKey
    ...
    await ctx.db.patch(item._id, {
      ...
      reportSendKey: idempotencyKey,
      reportSendClaimId: args.claimId,
      reportSendClaimExpiresAt: now + 10 * 60_000,
      reportSendMessageId: undefined,
      reviewedBy: admin.email,
      reviewedAt: now,
      sendError: undefined,
    });
    return { claimId: args.claimId, idempotencyKey, revision };
  },
});
export const finalizeReportSend = mutation({
  args: { assessmentId: v.string(), claimId: v.string(), messageId: v.string() },
  handler: async (ctx, args) => {
    await requireAdminIdentity(ctx);
    const item = await assessmentById(ctx, args.assessmentId);
    if (item.reportSendClaimId !== args.claimId || item.reportStatus !== "sending")
      throw new Error("CONFLICT: send claim does not match");
    const now = Date.now();
    await ctx.db.patch(item._id, {
      reportStatus: "sent",
      sentAt: now,
      reportSendMessageId: args.messageId,
      reportSendClaimExpiresAt: undefined,
      sendError: undefined,
    });
    return { sentAt: now, revision: item.reportRevision ?? 0 };
  },
});
export const failReportSend = mutation({
  args: { assessmentId: v.string(), claimId: v.string(), publicError: v.string() },
  handler: async (ctx, args) => {
    await requireAdminIdentity(ctx);
    const item = await assessmentById(ctx, args.assessmentId);
    if (item.reportSendClaimId !== args.claimId || item.reportStatus !== "sending") return false;
    await ctx.db.patch(item._id, {
      reportStatus: "send_failed",
      reportSendClaimExpiresAt: undefined,
      sendError: args.publicError.slice(0, 200),
    });
    return true;
  },
});
    ...
    return (
      (
        await ctx.db
          ...
          .withIndex("by_key", (q) => q.eq("key", "voice.defaultProvider"))
          .unique()
      )?.value || "ultravox"
    );
  },
});
export const adminGetDefaultProvider = query({
  args: {},
  handler: async (ctx) => {
    await requireAdminIdentity(ctx);
    return (
...
export const adminSetDefaultProvider = mutation({
