...
test("claim lease prevents concurrent workers from taking one delivery", async () => {
  ...
  const first = await t.mutation(internal.webhookDelivery.claimDue, {
    now: 1_000,
    leaseId: "lease-a",
  });
  const second = await t.mutation(internal.webhookDelivery.claimDue, {
    now: 1_000,
    leaseId: "lease-b",
  });
...
test("expired leases are recoverable and stale workers cannot finish", async () => {
  ...
  const recovered = await t.mutation(internal.webhookDelivery.claimDue, {
    now: 61_001,
    leaseId: "lease-b",
  });
  expect(recovered).toHaveLength(1);
  expect(
    await t.mutation(internal.webhookDelivery.finish, {
      deliveryId,
      leaseId: "lease-a",
      attempt: 1,
      success: true,
  ...
  const history = await t.run((ctx) =>
    ctx.db
      ...
      .withIndex("by_delivery_id_and_attempt", (q) =>
        q.eq("deliveryId", deliveryId).eq("attempt", 1),
      )
      .unique(),
  );
...
test("failed attempts persist sanitized history and prescribed retry delay", async () => {
  ...
  expect(delivery).toMatchObject({
    status: "pending",
    attempts: 1,
    nextAttemptAt: 71_000,
    lastError: "NETWORK_ERROR secret",
  });
...
test("manual pending delivery preserves attempt count and event id", async () => {
  const { t, deliveryId } = await deliveryFixture();
  await t.run((ctx) =>
    ctx.db.patch(deliveryId, { status: "manual_pending", attempts: 3, nextAttemptAt: 2_000 }),
  );
  const claimed = await t.mutation(internal.webhookDelivery.claimDue, {
    now: 2_000,
