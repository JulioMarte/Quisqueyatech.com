...
function requiredEnvironment(name: "SITE_URL" | "BETTER_AUTH_SECRET") {
  ...
  if (!value)
    throw new ConvexError({ code: "CONFIGURATION_ERROR", message: `${name} is required` });
  return value;
}
        ...
        create: {
          before: async (_user, context) => {
            if (!("runMutation" in ctx))
              throw new APIError("FORBIDDEN", { message: "Setup unavailable" });
          ...
          after: async (user, context) => {
            if (!("runMutation" in ctx))
              throw new APIError("INTERNAL_SERVER_ERROR", { message: "Setup failed" });
            let recoveryHashes: string[] = [];
            try {
              recoveryHashes = JSON.parse(header(context, "x-admin-recovery-hashes"));
            } catch {
              /* rejected below */
            }
...
export const claimSetup = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    else await ctx.db.insert("adminInstallation", { singleton: "admin", ...value });
    return { ok: true };
  },
});
...
export const finalizeSetup = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      (row.claimExpiresAt ?? 0) <= args.now ||
      args.recoveryHashes.length !== 8 ||
      args.recoveryHashes.some((value) => !/^[a-f0-9]{64}$/.test(value))
    )
      throw new ConvexError("Setup claim expired");
    await ctx.db.patch(row._id, {
      status: "configured",
      claimExpiresAt: undefined,
      adminUserId: args.userId,
      adminEmail: args.email.toLowerCase(),
      adminName: args.name,
      configuredAt: args.now,
      updatedAt: args.now,
    });
    for (const codeHash of args.recoveryHashes)
      await ctx.db.insert("adminRecoveryCodes", {
        userId: args.userId,
        codeHash,
        createdAt: args.now,
      });
...
export async function requireAdmin(
  ...
  const row: Doc<"adminInstallation"> | null = await ctx.runQuery(
    internal.auth.getInstallation,
    {},
  );
  if (!row || row.adminUserId !== user._id)
...
export const recoverAdmin = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.newPassword.length < 14 || args.newPassword.length > 128)
      throw new ConvexError("Invalid recovery request");
    const now = Date.now();
    const claimed: { userId: string; claimedAt: number } | null = await ctx.runMutation(
      internal.auth.claimRecoveryCode,
      { codeHash: args.codeHash, now },
    );
    ...
    try {
      const account = await ctx.runQuery(components.betterAuth.adapter.findOne, {
        model: "account",
        where: [
          { field: "userId", value: claimed.userId },
      ...
      await ctx.runMutation(components.betterAuth.adapter.updateOne, {
        input: {
          ...
          update: { password: await hashPassword(args.newPassword) },
        },
      });
      ...
      await ctx.runMutation(internal.auth.consumeRecoveryCode, {
      ...
      await ctx.runMutation(internal.auth.releaseRecoveryCode, {
        codeHash: args.codeHash,
        claimedAt: claimed.claimedAt,
      });
      throw error;
...
export const claimRecoveryCode = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      !code ||
      code.consumedAt ||
      (code.claimExpiresAt ?? 0) > args.now ||
      installation?.adminUserId !== code.userId
    )
      return null;
...
export const consumeRecoveryCode = internalMutation({
  args: { codeHash: v.string(), claimedAt: v.number(), now: v.number() },
  handler: async (ctx, args) => {
    const code = await ctx.db
      .query("adminRecoveryCodes")
      .withIndex("by_code_hash", (q) => q.eq("codeHash", args.codeHash))
      .unique();
    if (code?.claimedAt === args.claimedAt && !code.consumedAt)
      await ctx.db.patch(code._id, { consumedAt: args.now, claimExpiresAt: undefined });
  },
});
export const releaseRecoveryCode = internalMutation({
  args: { codeHash: v.string(), claimedAt: v.number() },
  handler: async (ctx, args) => {
    const code = await ctx.db
      .query("adminRecoveryCodes")
      .withIndex("by_code_hash", (q) => q.eq("codeHash", args.codeHash))
      .unique();
    if (code?.claimedAt === args.claimedAt && !code.consumedAt)
      await ctx.db.patch(code._id, { claimedAt: undefined, claimExpiresAt: undefined });
  },
});
...
export const listAgents = query({
  ...
  handler: async (ctx) => {
    ...
    const now = Date.now();
    return (await ctx.db.query("contentAgents").order("desc").take(200)).map((agent) => ({
      _id: agent._id,
      keyId: agent.keyId,
      name: agent.name,
      prefix: agent.prefix,
...
export const createAgent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    return ctx.db.insert("contentAgents", {
      keyId: args.keyId,
      name: args.name,
      tokenHash: `pending:${args.keyId}`,
      prefix: "pending",
      status: "pending",
      ...
      createdAt: now,
      pendingTokenHash: args.tokenHash,
      pendingPrefix: args.prefix,
      pendingActivationId: args.activationId,
      pendingExpiresAt: now + 30 * 60_000,
    });
  },
});
...
export const prepareAgentRotation = mutation({
  args: { keyId: v.string(), tokenHash: v.string(), prefix: v.string(), activationId: v.string() },
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const row = await ctx.db
      .query("contentAgents")
    ...
    const now = Date.now();
    if (row.pendingTokenHash && (row.pendingExpiresAt ?? 0) > now)
      throw new ConvexError({
        code: "CONFLICT",
        message: "A credential rotation is already pending",
      });
    await ctx.db.patch(row._id, {
      pendingTokenHash: args.tokenHash,
      pendingPrefix: args.prefix,
...
export const activateAgentCredential = mutation({
  args: { keyId: v.string(), activationId: v.string() },
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const row = await ctx.db
      .query("contentAgents")
    ...
    if (
      ...
      !row.pendingPrefix ||
      row.pendingActivationId !== args.activationId ||
      (row.pendingExpiresAt ?? 0) <= now
    )
      throw new ConvexError({
        code: "CONFLICT",
        message: "Pending credential expired or does not match",
      });
    await ctx.db.patch(row._id, {
      tokenHash: row.pendingTokenHash,
      prefix: row.pendingPrefix,
      status: "active",
      revokedAt: undefined,
      rotatedAt: row.status === "active" ? now : undefined,
      pendingTokenHash: undefined,
      pendingPrefix: undefined,
      pendingActivationId: undefined,
      pendingExpiresAt: undefined,
...
export const updateAgentLimits = mutation({
  args: { keyId: v.string(), requestLimit: v.number(), uploadLimit: v.number() },
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const row = await ctx.db
      .query("contentAgents")
    ...
    if (!row) return false;
    if (
      args.requestLimit < 10 ||
      args.requestLimit > 1000 ||
      args.uploadLimit < 1 ||
      args.uploadLimit > 100
    )
      throw new ConvexError({ code: "VALIDATION_ERROR", message: "Invalid limits" });
    await ctx.db.patch(row._id, {
      requestLimit: Math.floor(args.requestLimit),
      uploadLimit: Math.floor(args.uploadLimit),
    });
    return true;
  },
});
...
export const checkSecurityRateLimit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      args.limit > 10_000 ||
      args.windowMs < 1_000 ||
      args.windowMs > 7 * 24 * 60 * 60_000
    )
      throw new ConvexError("Invalid rate limit");
    const row = await ctx.db
    ...
    if (!row || row.resetAt <= now) {
      if (row) await ctx.db.patch(row._id, { count: 1, resetAt: now + args.windowMs });
        ...
        await ctx.db.insert("authSecurityRateLimits", {
          key: args.key,
          count: 1,
    ...
    if (row.count >= args.limit)
      return { allowed: false, retryAfter: Math.max(1, Math.ceil((row.resetAt - now) / 1000)) };
...
export const authenticateAgent = mutation({
  ...
  handler: async (ctx, args) => {
    requireAdminApiSecret(args.secret);
    const agent = await ctx.db
      .query("contentAgents")
      .withIndex("by_token_hash", (q) => q.eq("tokenHash", args.tokenHash))
      .unique();
    if (!agent || agent.status !== "active") return { status: "unauthorized" as const };
    const window = Math.floor(args.now / 3_600_000);
    const rateKey = `${agent.keyId}:${args.operation}:${window}`;
    const rate = await ctx.db
      .query("contentAgentRateLimits")
      .withIndex("by_key", (q) => q.eq("key", rateKey))
      .unique();
    const limit = args.operation === "upload" ? agent.uploadLimit : agent.requestLimit;
    if (rate && rate.count >= limit)
      return {
        status: "limited" as const,
        retryAfter: Math.ceil((rate.resetAt - args.now) / 1000),
      };
    if (rate) await ctx.db.patch(rate._id, { count: rate.count + 1 });
    else
      await ctx.db.insert("contentAgentRateLimits", {
        key: rateKey,
        count: 1,
        resetAt: (window + 1) * 3_600_000,
      });
    if (!agent.lastUsedAt || args.now - agent.lastUsedAt > 15 * 60_000)
      await ctx.db.patch(agent._id, { lastUsedAt: args.now });
...
export const cleanup = internalMutation({
  ...
  handler: async (ctx) => {
    const now = Date.now();
    for (const rate of await ctx.db
      .query("contentAgentRateLimits")
      .withIndex("by_reset_at", (q) => q.lte("resetAt", now - 60 * 60_000))
      .take(100))
    ...
    for (const agent of await ctx.db
      .query("contentAgents")
      .withIndex("by_pending_expiry", (q) => q.lte("pendingExpiresAt", now))
      .take(100)) {
