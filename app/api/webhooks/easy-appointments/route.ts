...
export async function POST(request: Request) {
  ...
  const result = await convexMutation("bookings:fromWebhook", { eventId, event, payload: JSON.stringify(payload), requestId, receivedAt: Date.now() });
...
function hashPayload(payload: unknown) { return Buffer.from(JSON.stringify(payload)).toString("base64url").slice(0, 80); }
