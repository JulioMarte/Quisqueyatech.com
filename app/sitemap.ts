...
const enRoutes = [
  ...
  "/en/solutions/clinics",
  "/en/how-we-work",
  "/en/assessment",
  "/en/assessment/now",
  "/en/assessment/schedule",
  "/en/recursos",
  "/en/about",
  "/en/privacy",
  "/en/messaging-terms",
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const pages = [...esRoutes, ...enRoutes].map((path, index) => ({
    url: `${brand.siteUrl}${path}`,
    lastModified: new Date(),
    changeFrequency: "weekly" as const,
    priority: index === 0 ? 1 : 0.7,
  }));
  const posts = [...(await getPublishedPosts("es")), ...(await getPublishedPosts("en"))].map(
    (post) => ({
      url: `${brand.siteUrl}${post.locale === "en" ? "/en" : ""}/recursos/${post.slug}`,
