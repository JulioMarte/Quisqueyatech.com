...
export function normalizeVoiceStatus(value: unknown): VoiceStatus {
  ...
  if (status.includes("end") || status.includes("disconnect") || status.includes("closed")) return "ended";
  if (status.includes("listen") || status === "idle" || status.includes("connected")) return "listening";
