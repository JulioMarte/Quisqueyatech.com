...
export async function sendApprovedAssessmentReport(
  email: string,
  locale: "es" | "en",
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: process.env.RESEND_FROM_EMAIL || "QuisqueyaTech <evaluaciones@quisqueyatech.com>",
      to: [email],
      bcc: process.env.LEAD_TO_EMAIL ? [process.env.LEAD_TO_EMAIL] : undefined,
      subject: report.subject,
      html: renderAssessmentReportHtml(locale, report),
    }),
  });
  if (!response.ok) throw new Error(`Report email failed (${response.status})`);
  const payload = (await response.json().catch(() => null)) as { id?: unknown } | null;
  if (typeof payload?.id !== "string" || !payload.id)
    throw new Error("Report provider returned an invalid response");
  return { messageId: payload.id };
}

export function renderAssessmentReportHtml(locale: "es" | "en", report: AssessmentReport) {
  const section = (title: string, body: string) =>
    `<h2 style="color:#082f49;margin:28px 0 8px">${escapeHtml(title)}</h2><p style="line-height:1.7;color:#334155">${escapeHtml(body)}</p>`;
