...
export function easyAppointmentsConfigured() {
  return config() !== null;
}
...
async function easyFetch(url: string, init?: RequestInit) {
  ...
  try {
    response = await fetch(url, {
      ...init,
      headers: { Authorization: `Bearer ${c.token}`, Accept: "application/json", ...init?.headers },
      cache: "no-store",
      signal: AbortSignal.timeout(10_000),
    });
  } catch {
    throw new SchedulingError("The scheduling service did not respond", 503, true);
  }
  if (!response.ok)
    throw new SchedulingError(
      response.status === 409 || response.status === 422
        ? "That time is no longer available"
        : `Easy!Appointments request failed (${response.status})`,
      response.status,
      response.status >= 500,
    );
  return response;
...
export class EasyAppointmentsProvider implements SchedulingProvider {
  async listAvailability(date: string, timezone: string): Promise<AvailableSlot[]> {
    ...
    url.searchParams.set("providerId", c.providerId);
    url.searchParams.set("serviceId", c.serviceId);
    url.searchParams.set("date", date);
    const response = await easyFetch(url.toString());
    const times = (await response.json()) as string[];
    return times.map((time) => {
      const start = fromZonedTime(`${date} ${time}`, c.timezone);
      return { start: start.toISOString(), label: formatInTimeZone(start, timezone, "h:mm a") };
    });
  }
  ...
  async book(input: BookingInput) {
    ...
    const payload = {
      start: formatInTimeZone(start, c.timezone, "yyyy-MM-dd HH:mm:ss"),
      end: formatInTimeZone(end, c.timezone, "yyyy-MM-dd HH:mm:ss"),
      location: input.channel === "phone" ? "Phone" : "QuisqueyaTech voice assessment",
      ...
      providerId: Number(c.providerId),
      customer: {
    ...
    const response = await easyFetch(`${c.baseUrl}/index.php/api/v1/appointments`, {
      method: "POST",
