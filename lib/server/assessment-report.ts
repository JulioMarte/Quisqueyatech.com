...
export async function buildAssessmentReport(
  ...
  const usableFields = Object.values(snapshot?.fields || {}).filter(
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      model: String(
        runtime.assessmentReportModel || process.env.ASSESSMENT_REPORT_MODEL || "gemini-2.5-flash",
      ),
          ...
          properties: {
            ...
            opportunities: {
              ...
              items: {
                ...
                required: ["title", "rationale", "impact", "confidence"],
              },
            },
      ...
      opportunities: [
        {
          ...
          rationale:
            "La evidencia reunida permite preparar una oportunidad de alcance controlado después de validar los datos pendientes.",
          impact: value("impact") || "Impacto por validar",
          confidence: snapshot?.complete ? "medium" : "low",
        },
      ],
      assumptions: Object.values(snapshot?.fields || {})
        .filter((item) => item.status !== "confirmed")
        .map((item) => `${item.field}: ${item.value}`),
      openQuestions: snapshot?.essentialMissing.map((item) => `Confirmar ${item}`) || [],
      nextStep:
        "Revisar la ficha con QuisqueyaTech y validar los datos pendientes antes de recomendar una implementación.",
    };
  return {
    ...
    opportunities: [
      {
        ...
        rationale:
          "The collected evidence supports a controlled opportunity after remaining facts are validated.",
    ...
    assumptions: Object.values(snapshot?.fields || {})
      .filter((item) => item.status !== "confirmed")
      .map((item) => `${item.field}: ${item.value}`),
    openQuestions: snapshot?.essentialMissing.map((item) => `Confirm ${item}`) || [],
    nextStep:
      "Review the brief with QuisqueyaTech and validate pending facts before recommending an implementation.",
  };
}
