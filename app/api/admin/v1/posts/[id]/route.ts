...
export async function GET(request: Request, context: Context) {
  ...
  try {
    ...
    const revisions =
      url.searchParams.get("include") === "revisions"
        ? await fetchAuthQuery(api.posts.adminRevisions, { postId: id as Id<"posts"> })
        : undefined;
    return adminJson(trace, { post, revisions });
  } catch (error) {
    return adminException(trace, "posts.revisions", error);
  }
}
...
export async function PATCH(request: Request, context: Context) {
  ...
  try {
    if (!(await authorizeContentRequest(request))) return adminFailure(trace, "Unauthorized", 401);
    const { id } = await context.params;
    const body = (await readAdminJson(request, 128_000)) as Record<string, unknown>;
    if (body?.revisionId) {
      if (typeof body.expectedUpdatedAt !== "number")
        return adminFailure(trace, "expectedUpdatedAt is required", 400);
      const data = await fetchAuthMutation(api.posts.adminRestore, {
        postId: id as Id<"posts">,
        revisionId: body.revisionId as Id<"postRevisions">,
        expectedUpdatedAt: body.expectedUpdatedAt,
      });
      return adminJson(trace, { id: data });
    }
    ...
    if (parsed.data.expectedUpdatedAt === undefined)
      return adminFailure(trace, "expectedUpdatedAt is required", 400);
    ...
    return message.includes("already uses")
      ? adminFailure(trace, message, 409)
      : adminException(trace, "posts.update", error);
  }
}
...
export async function DELETE(request: Request, context: Context) {
  ...
  try {
    if (!(await authorizeContentRequest(request))) return adminFailure(trace, "Unauthorized", 401);
    const { id } = await context.params;
    await fetchAuthMutation(api.posts.adminArchive, { id: id as Id<"posts"> });
    return adminJson(trace, { id, status: "archived" });
  } catch (error) {
    return adminException(trace, "posts.archive", error);
  }
}
