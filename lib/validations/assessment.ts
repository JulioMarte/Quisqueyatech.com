...
export const bookingSchema = z.object({
  firstName: z.string({ error: "El nombre es obligatorio." }).trim().min(2, "Escribe un nombre válido.").max(60),
  lastName: z.string({ error: "El apellido es obligatorio." }).trim().min(2, "Escribe un apellido válido.").max(80),
  company: z.string().trim().max(120).optional(),
  role: z.string().trim().max(100).optional(),
  country: z.string({ error: "El país es obligatorio." }).trim().min(2).max(80),
  locale: z.enum(["es", "en"]),
  email: z.string({ error: "El correo es obligatorio." }).trim().email("Escribe un correo válido.").max(160),
  phone,
  ...
  timezone: z.string().min(1).max(80).refine(
    (value) => {
      try {
...
export const agendaRulesSchema = z.object({
  ...
  for (const [index, rule] of value.weekly.entries()) {
    if (seen.has(rule.weekday)) context.addIssue({ code: "custom", path: ["weekly", index, "weekday"], message: "El día está repetido." });
    seen.add(rule.weekday);
    if (rule.start >= rule.end) context.addIssue({ code: "custom", path: ["weekly", index, "end"], message: "La hora final debe ser posterior a la inicial." });
  }
});

export const agendaExceptionSchema = z.object({
  ...
  if ((value.start === undefined) !== (value.end === undefined)) context.addIssue({ code: "custom", path: [value.start === undefined ? "start" : "end"], message: "Debes indicar ambas horas." });
  if (value.start !== undefined && value.end !== undefined && value.start >= value.end) context.addIssue({ code: "custom", path: ["end"], message: "La hora final debe ser posterior a la inicial." });
});
