...
const input = z.object({
  ...
  requestLimit: z.number().int().min(10).max(1000).default(120),
  uploadLimit: z.number().int().min(1).max(100).default(10),
});
function credential(keyId: string) {
  const raw = `qta_${keyId}_${randomBytes(32).toString("base64url")}`;
  return { raw, hash: tokenHash(raw), prefix: `${raw.slice(0, 18)}…` };
}
export async function GET(request: Request) {
  const trace = requestId(request);
  try {
    if (!(await authorizeContentRequest(request))) return adminFailure(trace, "Unauthorized", 401);
    return adminJson(trace, await fetchAuthQuery(api.auth.listAgents, {}));
  } catch (error) {
    return adminException(trace, "agents.list", error);
  }
}
export async function POST(request: Request) {
  ...
  try {
    ...
    const keyId = randomBytes(8).toString("base64url"),
      activationId = randomBytes(18).toString("base64url"),
      key = credential(keyId);
    await fetchAuthMutation(api.auth.createAgent, {
      keyId,
    ...
    return adminJson(
      trace,
      { keyId, token: key.raw, activationId, status: "pending", expiresInSeconds: 1800 },
      { status: 201 },
    );
  } catch (error) {
    return adminException(trace, "agents.create", error);
  }
}
