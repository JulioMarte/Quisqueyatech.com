...
import { ArrowRight, Bot, CalendarClock, Code2, HeartPulse, Mic2, Workflow } from "lucide-react";
...
export async function MarketingPage({ kind, locale, postSlug }: { kind: PageKind; locale: Locale; postSlug?: string }) {
  ...
  if (kind === "assessment") {
    return <><PageHero page={page} /><Section className="pt-0"><Container className="grid gap-5 md:grid-cols-2"><ChoiceCard icon={Mic2} title={locale === "es" ? "Hacerla ahora" : "Start now"} body={locale === "es" ? "Habla con el agente desde tu navegador y recibe el resumen al terminar." : "Speak with the agent in your browser and receive the summary when finished."} href={nowPath} action={locale === "es" ? "Comenzar evaluación" : "Start assessment"} /><ChoiceCard icon={CalendarClock} title={locale === "es" ? "Reservar un horario" : "Reserve a time"} body={locale === "es" ? "Elige navegador o llamada y realiza la misma evaluación cuando te convenga." : "Choose browser or phone and complete the same assessment when convenient."} href={schedulePath} action={locale === "es" ? "Ver disponibilidad" : "View availability"} /></Container></Section></>;
  }
  return <><PageHero page={page} /><Section className="pt-0"><Container><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{page.points.map(([title, body], index) => { const Icon = icons[index % icons.length]; return <article key={title} className="rounded-xl border border-line bg-white p-7"><Icon className="h-10 w-10 rounded-lg bg-larimar-soft p-2 text-larimar-deep" /><h2 className="mt-5 font-display text-xl font-bold">{title}</h2><p className="mt-2 leading-relaxed text-text-2">{body}</p></article>; })}</div><div className="mt-12 rounded-2xl bg-primary p-8 text-white sm:p-10"><h2 className="font-display text-3xl font-bold">{locale === "es" ? "Empieza por entender qué conviene resolver primero." : "Start by understanding what is worth solving first."}</h2><div className="mt-6 flex flex-col gap-3 sm:flex-row"><ButtonLink href={nowPath}>{locale === "es" ? "Quiero mi evaluación ahora" : "Start my assessment"}</ButtonLink><ButtonLink href={schedulePath} className="border border-white/25 bg-white/10 text-white hover:bg-white/15">{locale === "es" ? "Agendar mi evaluación" : "Schedule my assessment"}</ButtonLink></div></div></Container></Section></>;
}

function PageHero({ page }: { page: { eyebrow: string; title: string; lede: string } }) { return <Section className="bg-bg-2 py-20"><Container className="max-w-[960px]"><Eyebrow>{page.eyebrow}</Eyebrow><h1 className="mt-4 max-w-[20ch] font-display text-[clamp(38px,5vw,60px)] font-extrabold leading-tight tracking-[-.035em] text-primary">{page.title}</h1><p className="mt-5 max-w-[62ch] text-xl leading-relaxed text-text-2">{page.lede}</p></Container></Section>; }

function ChoiceCard({ icon: Icon, title, body, href, action }: { icon: typeof Mic2; title: string; body: string; href: string; action: string }) { return <article className="rounded-2xl border border-line bg-white p-8 shadow-sm"><Icon className="h-12 w-12 rounded-xl bg-primary p-2.5 text-white" /><h2 className="mt-6 font-display text-2xl font-bold">{title}</h2><p className="mt-3 leading-relaxed text-text-2">{body}</p><ButtonLink href={href} className="mt-6">{action}<ArrowRight className="h-4 w-4" /></ButtonLink></article>; }
