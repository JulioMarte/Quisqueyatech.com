...
const solutionLinks = {
  ...
  en: [
    ...
    ["/en/solutions/software-and-integrations", "Software and integrations", "Connect isolated tools and data."],
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-line bg-white/95 backdrop-blur-md">
      <Container className="grid h-[68px] grid-cols-[auto_1fr_auto] items-center gap-4">
        ...
        <nav className="hidden items-center justify-self-center gap-1 xl:flex" aria-label={locale === "es" ? "Principal" : "Main"}>
                ...
                <div className="grid grid-cols-2 gap-1 p-2">
                  {solutionLinks[locale].map(([href, label, description]) => (
                    ...
                    >
                      ...
                      <span className="mt-1 block text-xs leading-relaxed text-mute">{description}</span>
                    </Link>
                  ))}
                </div>
                <Link href={solutionsHref} className="flex min-h-11 items-center justify-between border-t border-line bg-bg-2 px-5 text-sm font-semibold text-primary transition-colors hover:bg-larimar-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-larimar-deep">
      ...
      {open ? <m.div id={mobileMenuId} initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }} className="overflow-hidden border-t border-line bg-white shadow-lg xl:hidden">
        <Container className="max-h-[calc(100dvh-68px)] overflow-y-auto py-4">
          ...
          {solutionsOpen ? <m.div id={mobileSolutionsId} initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }} className="mt-1 overflow-hidden border-l-2 border-larimar-soft pl-2">
            ...
            <Link href={solutionsHref} onClick={closeMobileMenu} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-sm font-semibold text-primary hover:bg-bg-2">
          ...
          {mainLinks.map((link) => (
            <Link key={link.href} href={link.href} onClick={closeMobileMenu} aria-current={isActive(link.href) ? "page" : undefined} className={cn("flex min-h-11 items-center rounded-lg px-3 text-sm font-medium transition-colors hover:bg-bg-2", isActive(link.href) ? "bg-bg-2 font-semibold text-primary" : "text-text-2")}>{link.label}</Link>
          ))}
...
function LanguageSwitch({
  ...
  return (
    <div className="inline-flex h-11 items-stretch overflow-hidden rounded-lg border border-line bg-bg-2" aria-label={locale === "es" ? "Seleccionar idioma" : "Select language"}>
      {locale === "es" ? (
        <>
          ...
          <Link href={alternateHref} hrefLang="en" onClick={onNavigate} className="inline-flex min-w-11 cursor-pointer items-center justify-center px-2 text-xs font-semibold text-mute transition-colors hover:bg-white hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-larimar-deep">EN</Link>
        </>
      ) : (
        <>
          <Link href={alternateHref} hrefLang="es" onClick={onNavigate} className="inline-flex min-w-11 cursor-pointer items-center justify-center px-2 text-xs font-semibold text-mute transition-colors hover:bg-white hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-larimar-deep">ES</Link>
