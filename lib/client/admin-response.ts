...
export class AdminRequestError extends Error {
  constructor(message: string, readonly status: number, readonly requestId?: string, readonly errorCode?: AdminErrorCode) {
    super(message);
...
export async function adminRequest<T>(input: RequestInfo | URL, init?: RequestInit): Promise<T> {
  ...
  try { response = await fetch(input, { ...init, signal: init?.signal ?? controller.signal }); }
  ...
  if (!response.ok || !payload || !("data" in payload)) {
    ...
    throw new AdminRequestError((payload?.error || `La operación administrativa falló (${response.status}).`) + reference, response.status, payload?.requestId, payload?.errorCode ?? undefined);
  }
