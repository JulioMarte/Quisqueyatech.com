...
async function loadRuntime(ctx: QueryCtx) {
  const config = await ctx.db
    .query("systemSettings")
    .withIndex("by_key", (q) => q.eq("key", "runtime.config"))
    .unique();
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      secrets: Object.fromEntries(
        secrets.map((item) => [
          item.key,
          { configured: true, lastFour: item.lastFour, updatedAt: item.updatedAt },
        ]),
      ),
    };
  },
...
export const adminSave = mutation({
  args: { config: v.any(), secrets: v.array(secretItem) },
  handler: async (ctx, args) => {
    ...
    if (
      args.secrets.some(
        (secret) =>
          !allowedSecretKeys.has(secret.key) ||
          secret.version !== 1 ||
    ...
    const changedFields = [...new Set([...Object.keys(previous), ...Object.keys(config)])]
      ...
      .sort();
    if (current) {
      await ctx.db.patch(current._id, { value: config, updatedBy: admin.email, updatedAt: now });
    } else {
    ...
    for (const secret of args.secrets) {
      const existing = await ctx.db
        .query("secretSettings")
        .withIndex("by_key", (q) => q.eq("key", secret.key))
        .unique();
      const value = { ...secret, updatedBy: admin.email, updatedAt: now };
      if (existing) await ctx.db.replace(existing._id, value);
      else await ctx.db.insert("secretSettings", value);
    }
    const changedSecretKeys = args.secrets.map((secret) => secret.key).sort();
    if (changedFields.length || changedSecretKeys.length) {
      await ctx.db.insert("configurationAudit", {
...
/**
 * Private runtime loader for Convex actions / HTTP machine gateway only.
 * Ciphertexts never leave Convex via a public query API.
 */
export const internalRuntime = internalQuery({
  args: {},
  handler: async (ctx) => loadRuntime(ctx),
});
