...
export function startOpenTelemetry() {
  if (sdk || !isOpenTelemetryEnabled()) return sdk;

  configureDiagnostics();

  const serviceName = process.env.OTEL_SERVICE_NAME || "quisqueyatech-livekit-agent";
  ...
  sdk = new NodeSDK({
    ...
    traceExporter: shouldExport("OTEL_TRACES_EXPORTER") ? new OTLPTraceExporter() : undefined,
...
function shouldExport(
  ...
  return process.env[variableName]?.trim().toLowerCase() !== "none";
}
...
function configureDiagnostics() {
  ...
  if (typeof level === "number") diag.setLogger(new DiagConsoleLogger(), level);
}
