...
export async function GET(request: Request, context: Context) {
  ...
  try {
    const data = await convexQuery("posts:serverRevisions", { postId: id });
...
export async function PATCH(request: Request, context: Context) {
  ...
  if (body.revisionId) {
    try { const data = await convexMutation("posts:serverRestore", { revisionId: body.revisionId }); return NextResponse.json({ data: { id: data }, error: null, requestId: trace }); }
  ...
  try { const data = await convexMutation("posts:serverSave", { ...parsed.data, actorType: "admin", actorId: actor.id, actorLabel: actor.label }); return NextResponse.json({ data: { id: data }, error: null, requestId: trace }); }
...
export async function DELETE(request: Request, context: Context) {
  ...
  if (!(await authorizeContentRequest(request))) return NextResponse.json({ data: null, error: "Unauthorized", requestId: trace }, { status: 401 });
  const { id } = await context.params;
  try { await convexMutation("posts:serverArchive", { id }); return NextResponse.json({ data: { id, status: "archived" }, error: null, requestId: trace }); }
