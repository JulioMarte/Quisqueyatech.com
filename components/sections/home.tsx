...
const copy = {
  es: {
    ...
    problemTitle: "El trabajo se frena cuando procesos, personas y herramientas operan por separado.",
    problemLede: "No recomendamos software por moda. Buscamos dónde se repite trabajo, se pierde información o una decisión llega demasiado tarde.",
    problems: [
      ["Trabajo manual que se multiplica", "Datos copiados entre formularios, hojas de cálculo y sistemas que nunca se sincronizan."],
      ["Seguimientos que dependen de memoria", "Solicitudes, clientes y tareas importantes quedan sin un próximo paso visible."],
      ["Información difícil de convertir en decisiones", "Los reportes llegan tarde o requieren horas de preparación manual."],
    ],
    solutionsEyebrow: "Soluciones",
    solutionsTitle: "Tecnología aplicada a un resultado operativo claro.",
    solutionsLede: "Podemos empezar con una mejora puntual o diseñar un sistema por fases. La herramienta se elige después de entender el proceso.",
    solutionCards: [
      ["Automatización de procesos", "Conecta tareas repetitivas, aprobaciones, seguimiento y reportes para que el trabajo avance sin persecución constante.", "/soluciones/automatizacion"],
      ["Agentes de IA", "Agentes de voz o texto que recopilan información, atienden solicitudes y ejecutan tareas con límites definidos.", "/soluciones/agentes-de-ia"],
      ["Software e integraciones", "Aplicaciones e integraciones que unen los datos y herramientas que tu empresa ya utiliza.", "/soluciones/software-e-integraciones"],
    ],
    assessmentEyebrow: "Empieza con claridad",
    assessmentTitle: "Una evaluación que también demuestra lo que podemos construir.",
    assessmentBody: "Nuestro agente de voz realiza un levantamiento estructurado de 12–15 minutos. Al terminar recibes tres oportunidades preliminares, su impacto esperado y un próximo paso claro.",
    ...
    steps: [
      ...
      ["02", "Priorizar", "Separamos oportunidades reales de ideas que todavía no justifican inversión."],
    ...
    founderBody: "Julio lidera las evaluaciones, el diseño de soluciones y la relación con cada empresa. QuisqueyaTech trabaja desde República Dominicana con empresas dentro y fuera del país.",
    resources: "Recursos para tomar mejores decisiones tecnológicas",
    resourcesBody: "Guías honestas sobre automatización, agentes de IA e integración de sistemas. Sin tendencias vacías ni promesas infladas.",
    read: "Leer guía",
  ...
  en: {
    ...
    problemLede: "We do not recommend software because it is trending. We look for repeated work, missing information, and decisions that arrive too late.",
    problems: [
      ["Manual work keeps multiplying", "Data is copied between forms, spreadsheets, and systems that never stay in sync."],
      ["Follow-up depends on memory", "Requests, customers, and important tasks have no visible next step."],
      ["Information is hard to turn into decisions", "Reports arrive late or take hours of manual preparation."],
    ],
    solutionsEyebrow: "Solutions",
    solutionsTitle: "Technology tied to a clear operational outcome.",
    solutionsLede: "Start with one targeted improvement or build a system in phases. We choose the technology after understanding the process.",
    solutionCards: [
      ["Process automation", "Connect recurring tasks, approvals, follow-up, and reporting so work keeps moving.", "/en/solutions/automation"],
      ["AI agents", "Voice and text agents that collect information, handle requests, and perform tasks within defined boundaries.", "/en/solutions/ai-agents"],
      ["Software and integrations", "Applications and integrations that connect the data and tools your company already uses.", "/en/solutions/software-and-integrations"],
    ],
    assessmentEyebrow: "Start with clarity",
    assessmentTitle: "An assessment that also demonstrates what we can build.",
    assessmentBody: "Our voice agent runs a structured 12–15 minute discovery session. You receive three preliminary opportunities, expected impact, and a clear next step.",
    ...
    steps: [
      ...
      ["02", "Prioritize", "Separate real opportunities from ideas that do not justify investment yet."],
    ...
    founderBody: "Julio leads assessments, solution design, and each client relationship. QuisqueyaTech works from the Dominican Republic with companies at home and abroad.",
    resources: "Resources for better technology decisions",
    resourcesBody: "Practical guidance on automation, AI agents, and systems integration—without empty trends or inflated promises.",
    read: "Read guide",
  ...
  return <>
    <Section className="hero-orchestration overflow-hidden py-16 sm:py-20">
      ...
      <Container className="max-w-[1080px] text-center">
        <div className="hero-support [--hero-delay:.02s]"><Eyebrow>{c.eyebrow}</Eyebrow></div>
        <KineticText
          titleA={c.titleA}
          titleB={c.titleB}
        ...
        <SystemFlow
          labels={locale === "es" ? ["Automatización", "Agentes de IA", "Software"] : ["Automation", "AI agents", "Software"]}
    ...
    <Section className="bg-bg-2">
      <Container>
        ...
        <div className="grid gap-4 lg:grid-cols-3">{c.solutionCards.map(([title, body, href], index) => { const Icon = solutionIcons[index]; return <MotionCard key={title} index={index}><Link href={href} className="interactive-card group block h-full rounded-xl border border-line bg-white p-7"><Icon className="h-10 w-10 rounded-lg bg-primary p-2 text-white transition-transform duration-300 group-hover:-translate-y-1 group-hover:rotate-3 group-hover:scale-105" /><h2 className="mt-5 font-display text-xl font-bold">{title}</h2><p className="mt-2 min-h-24 leading-relaxed text-text-2">{body}</p><span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-amber-deep">{locale === "es" ? "Explorar solución" : "Explore solution"}<ArrowRight className="motion-arrow h-4 w-4" /></span></Link></MotionCard>; })}</div>
      </Container>
    </Section>
    ...
    <Section>
      <Container><SectionHead eyebrow={c.methodEyebrow} title={c.methodTitle} /><ScrollProgressSteps steps={c.steps} /></Container>
    </Section>
