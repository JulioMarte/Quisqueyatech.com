...
export async function GET(request: Request) {
  ...
  try { return NextResponse.json({ data: await convexQuery("assessments:adminList", { secret: adminSecret() }) || [], error: null, requestId: id }); }
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (parsed.data.action === "save") {
      await convexMutation("assessments:adminReview", { secret: adminSecret(), assessmentId: parsed.data.assessmentId, reportDraft: parsed.data.report, reportStatus: "review_pending", reviewedBy: actor.email, reviewedAt: Date.now() });
      return NextResponse.json({ data: { sent: false }, error: null, requestId: id });
    }
    const claim = await convexMutation("assessments:claimReportSend", { secret: adminSecret(), assessmentId: parsed.data.assessmentId, reviewedBy: actor.email, now: Date.now() }) as { revision: number } | null;
    ...
    try {
      ...
      await convexMutation("assessments:adminReview", { secret: adminSecret(), assessmentId: parsed.data.assessmentId, reportDraft: parsed.data.report, reportStatus: "sent", reviewedBy: actor.email, reviewedAt: sentAt, sentAt });
    } catch (error) {
      await convexMutation("assessments:adminReview", { secret: adminSecret(), assessmentId: parsed.data.assessmentId, reportDraft: parsed.data.report, reportStatus: "send_failed", reviewedBy: actor.email, reviewedAt: Date.now(), sendError: error instanceof Error ? error.message : "Email failed" });
      throw error;
    }
