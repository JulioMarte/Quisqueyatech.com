...
export function RecoveryForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(body.error || "No se pudo recuperar la cuenta.");
        return;
      }
  ...
  return (
    ...
    >
      <h1 className="font-display text-3xl font-bold text-primary">Recuperar acceso</h1>
      <p className="mt-2 text-sm text-text-2">
        Usa uno de los códigos guardados durante la instalación.
      </p>
      <label className="mt-6 block text-sm font-semibold" htmlFor="code">
        Código de recuperación
      </label>
      <input className={field} id="code" name="code" autoComplete="one-time-code" required />
      ...
      <Link
        href="/sign-in"
        className="mt-4 block min-h-11 py-3 text-center text-sm font-semibold text-tech"
      >
        Volver al acceso
      </Link>
    </form>
