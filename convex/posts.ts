...
export const serverSave = mutation({
  args: { secret: v.string(), ...postFields, actorType: v.optional(actor.actorType), actorId: v.optional(v.string()), actorLabel: v.optional(v.string()) },
  handler: async (ctx, args) => {
...
export const agentList = query({ args: { secret: v.string() }, handler: async (ctx, args) => { requireServer(args.secret); return (await ctx.db.query("posts").order("desc").take(500)).filter(post => post.status === "draft" || post.status === "review_pending").sort((a, b) => b.updatedAt - a.updatedAt); } });
export const agentGet = query({ args: { secret: v.string(), id: v.id("posts") }, handler: async (ctx, args) => { requireServer(args.secret); const post = await ctx.db.get(args.id); return post && (post.status === "draft" || post.status === "review_pending") ? post : null; } });
export const agentSave = mutation({
  args: { secret: v.string(), ...postFields, expectedUpdatedAt: v.optional(v.number()), ...actor },
  handler: async (ctx, args) => {
    requireServer(args.secret); const { secret: _secret, id, expectedUpdatedAt, actorType, actorId, actorLabel, status: _status, publishedAt: _publishedAt, ...input } = args; void _secret; void _status; void _publishedAt;
...
export const serverRegisterMedia = mutation({ args: { secret: v.string(), storageId: v.id("_storage"), filename: v.string(), contentType: v.string(), purpose: v.string(), ownerLabel: v.optional(v.string()) }, handler: async (ctx, args) => { requireServer(args.secret); const metadata = await ctx.db.system.get("_storage", args.storageId); const accepted = ["image/jpeg", "image/png", "image/webp", "image/avif"]; if (!metadata || metadata.size > 5_000_000 || !metadata.contentType || metadata.contentType !== args.contentType || !accepted.includes(metadata.contentType)) { if (metadata) await ctx.storage.delete(args.storageId); throw new Error("Invalid media upload"); } return ctx.db.insert("media", { storageId: args.storageId, filename: args.filename, contentType: args.contentType, purpose: args.purpose, ownerEmail: args.ownerLabel || configuredAdminEmail(), createdAt: Date.now() }); } });
export const serverRecordAiRun = mutation({ args: { secret: v.string(), postId: v.optional(v.id("posts")), action: v.string(), provider: v.string(), model: v.string(), status: v.string(), warnings: v.array(v.string()), inputTokens: v.optional(v.number()), outputTokens: v.optional(v.number()), durationMs: v.number() }, handler: async (ctx, args) => { requireServer(args.secret); const { secret: _secret, ...run } = args; void _secret; return ctx.db.insert("aiRuns", { ...run, actorEmail: configuredAdminEmail(), createdAt: Date.now() }); } });
...
export const backfillActors = mutation({
  args: { secret: v.string(), target: v.union(v.literal("posts"), v.literal("revisions")), paginationOpts: paginationOptsValidator },
  handler: async (ctx, args) => {
    requireServer(args.secret); let updated = 0;
