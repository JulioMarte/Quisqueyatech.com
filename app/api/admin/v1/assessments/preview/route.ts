...
export async function POST(request: Request) {
  ...
  try {
    ...
    return adminJson(trace, { html: renderAssessmentReportHtml(parsed.data.locale, parsed.data.report) });
  } catch (error) { return adminException(trace, "assessments.preview", error); }
}
