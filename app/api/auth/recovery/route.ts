...
export async function POST(request: Request) {
  ...
  try {
    const rate = await checkSecurityRateLimit(
      requestFingerprint(request, "admin-recovery"),
      5,
      60 * 60_000,
    );
    if (!rate.allowed) {
      return Response.json(
        { error: "Código o contraseña no válidos." },
        { status: 429, headers: { "Retry-After": String(rate.retryAfter) } },
      );
    }
  } catch (error) {
    return authConfigResponse(error, "auth:recovery:rate-limit");
  }
  const parsed = input.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return Response.json({ error: "Código o contraseña no válidos." }, { status: 400 });
  }
  try {
    await convexAction("auth:recoverAdmin", {
      serviceSecret: adminSecret(),
      codeHash: createHash("sha256").update(parsed.data.code.toUpperCase()).digest("hex"),
      newPassword: parsed.data.password,
    });
    console.info(JSON.stringify({ scope: "auth", operation: "recovery", status: "success" }));
    return Response.json({ data: { ok: true } });
  } catch {
    console.warn(JSON.stringify({ scope: "auth", operation: "recovery", status: "rejected" }));
    return Response.json({ error: "Código o contraseña no válidos." }, { status: 401 });
  }
}
