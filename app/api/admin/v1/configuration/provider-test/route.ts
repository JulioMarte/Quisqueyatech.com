...
const requestSchema = z.object({ provider: z.enum(["ultravox", "livekit", "gemini-live"]) }).strict();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (parsed.data.provider === "gemini-live") {
      if (!config.geminiApiKey) return adminFailure(trace, "Gemini no tiene una API key configurada.", 503, "CONFIGURATION_ERROR");
      ...
      return adminJson(trace, { provider: parsed.data.provider, success: true, durationMs: Date.now() - startedAt });
    }

    if (parsed.data.provider === "ultravox") {
      if (!config.ultravoxApiKey || !config.ultravoxApiUrl) return adminFailure(trace, "Ultravox no está configurado completamente.", 503, "CONFIGURATION_ERROR");
      ...
      if (!result.success) return adminFailure(trace, `Ultravox rechazó la prueba (${result.error || "UPSTREAM_ERROR"}).`, 502, "UPSTREAM_ERROR");
      return adminJson(trace, { provider: parsed.data.provider, success: true, statusCode: result.statusCode, durationMs: result.durationMs });
    }

    if (!config.livekitUrl || !config.livekitApiKey || !config.livekitApiSecret) return adminFailure(trace, "LiveKit no está configurado completamente.", 503, "CONFIGURATION_ERROR");
    ...
    if (!result.success) return adminFailure(trace, `LiveKit rechazó la prueba (${result.error || "UPSTREAM_ERROR"}).`, 502, "UPSTREAM_ERROR");
    return adminJson(trace, { provider: parsed.data.provider, success: true, statusCode: result.statusCode, durationMs: result.durationMs });
  } catch (error) {
