...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body?.storageId) {
      ...
      const id = await fetchAuthMutation(api.posts.adminRegisterMedia, { ...parsed.data, storageId: parsed.data.storageId as Id<"_storage"> });
      return adminJson(trace, { id });
    }
    const uploadUrl = await fetchAuthMutation(api.posts.adminGenerateUploadUrl, {});
    return adminJson(trace, { uploadUrl, maxBytes: 5_000_000, acceptedTypes: registration.shape.contentType.options });
  } catch (error) { return adminException(trace, "media.upload", error); }
}
