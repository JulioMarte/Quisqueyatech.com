...
export interface ContentAiProvider {
  generate(input: AiActionInput): Promise<{ proposal: AiProposal; model: string; inputTokens?: number; outputTokens?: number }>;
}
...
class GeminiContentProvider implements ContentAiProvider {
  async generate(input: AiActionInput) {
    ...
    if (input.action === "translate" && (!input.targetLocale || input.targetLocale === input.locale)) throw new Error("Translation requires a different targetLocale");
    const language = outputLocale === "es" ? "Spanish" : "English";
    const sourceRule = input.sources.length ? `User-provided references (not automatically verified): ${input.sources.join(", ")}. Do not claim you opened or verified them.` : "No verified sources were provided. Flag every factual or quantitative claim that requires verification.";
    ...
    const response = await ai.models.generateContent({
      model,
      contents: prompt,
      config: { responseMimeType: "application/json", responseJsonSchema: z.toJSONSchema(proposalSchema) },
    });
    const parsed = proposalSchema.parse(JSON.parse(response.text || "{}"));
    return { proposal: parsed, model, inputTokens: response.usageMetadata?.promptTokenCount, outputTokens: response.usageMetadata?.candidatesTokenCount };
  }
}
