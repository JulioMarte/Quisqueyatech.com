...
class GeminiContentProvider implements ContentAiProvider {
  async generate(input: AiActionInput) {
    ...
    const sourceRule = input.sources.length ? `Authorized sources: ${input.sources.join(", ")}.` : "No verified sources were provided. Flag every factual or quantitative claim that requires verification.";
