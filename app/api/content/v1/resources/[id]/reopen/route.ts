...
export async function POST(request: Request, context: Context) {
  const auth = await contentAgent(request, "transition");
  if (auth.response) return auth.response;
  const parsed = transitionSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success)
    return NextResponse.json({ error: parsed.error.issues[0]?.message }, { status: 400 });
  try {
    const data = await convexMutation("posts:agentTransition", {
      secret: adminSecret(),
    ...
    const message = error instanceof Error ? error.message : "Could not reopen";
