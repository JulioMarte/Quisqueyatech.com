...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!result.success) return adminFailure(trace, `El webhook de prueba falló (${result.error || "UPSTREAM_ERROR"}).`, 502, "UPSTREAM_ERROR");
    return adminJson(trace, result);
  } catch (error) { return adminException(trace, "configuration.webhook-test", error); }
}
