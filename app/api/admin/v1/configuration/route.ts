...
const updateSchema = z.object({ config: configurationSchema, secrets: secretsSchema.optional().default({}) }).strict();
...
export async function GET(request: Request) {
  ...
  } catch (error) { return adminException(trace, "configuration.get", error); }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const secrets = Object.entries(parsed.data.secrets)
      ...
      .map(([key, value]) => ({ key, ciphertext: encryptSetting(value), lastFour: maskLastFour(value), version: 1 }));
    await fetchAuthMutation(api.settings.adminSave, { config, secrets });
    return adminJson(trace, { ok: true });
  } catch (error) { return adminException(trace, "configuration.update", error); }
}
