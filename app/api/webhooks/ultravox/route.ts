...
export async function POST(request: Request) {
  ...
  if (!verifyWebhook(request, raw, String(runtime.ultravoxWebhookSecret || "")))
  ...
  const fresh = await convexMutation("assessments:recordWebhook", {
    eventId,
    provider: "ultravox",
    event: payload.event,
  ...
  try {
    const stored = (await convexQuery("assessments:getByProviderSession", {
      providerSessionId: payload.call.callId,
    })) as {
      assessmentId: string;
    ...
    const transcript = redactSensitiveText(
      await getUltravoxTranscript(payload.call.callId, runtime),
    ).text;
    ...
    const durationSeconds = Math.min(
      900,
      payload.call.ended && payload.call.created
        ? Math.round(
            (new Date(payload.call.ended).getTime() - new Date(payload.call.created).getTime()) /
              1000,
          )
        : snapshot.elapsedSeconds,
    );
    const progress = (await convexMutation("assessments:advance", {
      ...
      alerts: [],
      now: Date.now(),
    })) as { snapshot: AssessmentSnapshot };
    ...
    await convexMutation("assessments:finishWebhook", {
      eventId,
      success: false,
...
function verifyWebhook(request: Request, raw: string, secret: string) {
  ...
  const signatures = (request.headers.get("x-ultravox-webhook-signature") || "")
    .split(",")
    .map((item) => item.trim());
  const time = new Date(timestamp).getTime();
  if (!Number.isFinite(time) || Math.abs(Date.now() - time) > 60_000) return false;
  const expected = createHmac("sha256", secret)
    .update(raw + timestamp)
    .digest("hex");
...
async function getUltravoxTranscript(callId: string, runtime: RuntimeConfig) {
  ...
  const response = await fetch(`${base}/${encodeURIComponent(callId)}/messages`, {
    headers: { "X-API-Key": String(runtime.ultravoxApiKey) },
    cache: "no-store",
  });
  if (!response.ok) throw new Error(`Ultravox transcript failed (${response.status})`);
  const payload = (await response.json()) as
    { results?: { role?: string; text?: string }[] } | { role?: string; text?: string }[];
