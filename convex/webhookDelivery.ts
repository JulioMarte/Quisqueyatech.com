...
export const claimDue = internalMutation({
  ...
  handler: async (ctx, args): Promise<ClaimedDelivery[]> => {
    ...
    const manual = await ctx.db
      .query("webhookDeliveries")
      .withIndex("by_status_and_next_attempt_at", (q) =>
        q.eq("status", "manual_pending").lte("nextAttemptAt", args.now),
      )
      .take(10);
    const expired = await ctx.db
      ...
      .take(10);
    const selected = [...manual, ...pending, ...expired]
    ...
    for (const item of selected) {
      if (item.status === "processing" && item.attempts > 0) {
        const abandoned = await ctx.db
          ...
          .withIndex("by_delivery_id_and_attempt", (q) =>
            q.eq("deliveryId", item._id).eq("attempt", item.attempts),
          )
          .unique();
        if (abandoned && abandoned.completedAt === undefined)
...
export const finish = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const history = await ctx.db
      .query("webhookDeliveryAttempts")
      .withIndex("by_delivery_id_and_attempt", (q) =>
        q.eq("deliveryId", args.deliveryId).eq("attempt", args.attempt),
      )
      .unique();
    const error = args.error?.replace(/[\r\n\t]/g, " ").slice(0, 200);
    if (history?.leaseId === args.leaseId)
    ...
    if (args.success) {
      ...
      await ctx.db.patch(item._id, {
        ...common,
        status: "pending",
        nextAttemptAt: args.now + retryDelays[args.attempt - 1],
      });
    }
    return true;
  },
...
async function deliverClaim(
  ...
  const body = JSON.stringify({
    ...(claimed.item.payload as Record<string, unknown>),
...
export const processDue = internalAction({
  ...
  handler: async (ctx): Promise<number | null> => {
    ...
    const claimed: ClaimedDelivery[] = await ctx.runMutation(internal.webhookDelivery.claimDue, {
      now: Date.now(),
      leaseId,
    });
    await Promise.all(
      claimed.map((item) =>
        deliverClaim(ctx, item, config.webhookUrl!, runtime.secrets.webhookSecret),
      ),
    );
...
export const adminRetry = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (item.status === "processing" && (item.leaseExpiresAt ?? 0) > Date.now())
...
export const adminAttempts = query({
  ...
  handler: async (ctx, args) => {
    ...
    return ctx.db
      .query("webhookDeliveryAttempts")
      .withIndex("by_event_id_and_requested_at", (q) => q.eq("eventId", args.eventId))
      .order("desc")
      .take(50);
  },
});
...
export const adminTest = action({
  ...
  handler: async (
    ...
    const runtime = await ctx.runQuery(internal.settings.internalRuntime, {});
    const config = runtime.config as { webhookEnabled?: boolean; webhookUrl?: string };
    if (!config.webhookUrl || !runtime.secrets.webhookSecret)
      throw new Error("CONFIGURATION_ERROR: webhook URL and secret are required");
    ...
    const result: DeliveryResult = await ctx.runAction(internal.webhookHttp.deliver, {
      ...
      encryptedSecret: runtime.secrets.webhookSecret,
      eventId,
      body,
