...
export async function GET(request: Request) {
  ...
  try {
    ...
    const url = new URL(request.url); const cursor = url.searchParams.get("cursor"), search = url.searchParams.get("q")?.trim();
    const status = url.searchParams.get("status") as "draft" | "review_pending" | "scheduled" | "published" | "archived" | null;
    const locale = url.searchParams.get("locale") as "es" | "en" | null;
    if (search) { const items = await fetchAuthQuery(api.posts.adminSearch, { search: search.slice(0, 120), status: status || undefined, locale: locale || undefined }); return adminJson(id, { items, continueCursor: "", isDone: true }); }
    const page = await fetchAuthQuery(api.posts.adminList, { paginationOpts: { cursor, numItems: Math.min(50, Math.max(1, Number(url.searchParams.get("limit") || 30))) }, status: status || undefined, locale: locale || undefined });
    return adminJson(id, { items: page.page, continueCursor: page.continueCursor, isDone: page.isDone });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = await fetchAuthMutation(api.posts.adminSave, { ...parsed.data, id: parsed.data.id as Id<"posts"> | undefined, imageId: parsed.data.imageId as Id<"_storage"> | undefined, idempotencyKey });
